...
- 헤더(Header)의 전역 검색창 입력 버그 수정 및 검색 시 지도 페이지(MapMode)로 자동 이동하여 위치를 검색하도록 라우팅 기능 통합
- `RoadDamage` 데이터 모델 및 관련 로직 전체에 '이미지 내 파손 좌표(imageX, imageY)' 필드 추가 (Backend, Frontend, Docs 업데이트)
